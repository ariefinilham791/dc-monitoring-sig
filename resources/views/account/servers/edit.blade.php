@extends('layouts.base', ['title' => 'Edit Server'])
    ...
    <section class="position-relative overflow-hidden bg-gradient2 py-4 px-3">
        <div class="container">
            <div class="mb-4">
                ...
                <h3 class="mt-2 mb-0">Edit Server</h3>
                ...
                <div class="card-body p-3 p-md-4">
                    <form action="{{ route('server.update', $server) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('location_id') is-invalid @enderror" id="location_id" name="location_id" required>
                                ...
                                @foreach($locations as $loc)
                                    <option value="{{ $loc->id }}" {{ old('location_id', $server->location_id) == $loc->id ? 'selected' : '' }}>{{ $loc->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('os') is-invalid @enderror" id="os" name="os" value="{{ old('os', $server->os) }}" maxlength="100">
                            @error('os')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label d-block">Tipe Server</label>
                            <div class="status-pills">
                                <input type="radio" name="server_type" id="st_physical" value="physical" {{ old('server_type', $server->server_type) === 'physical' ? 'checked' : '' }} class="status-pill-input">
                                <label for="st_physical" class="status-pill status-pill-secondary">Physical</label>
                                <input type="radio" name="server_type" id="st_virtual" value="virtual" {{ old('server_type', $server->server_type) === 'virtual' ? 'checked' : '' }} class="status-pill-input">
                                <label for="st_virtual" class="status-pill status-pill-info">Virtual</label>
                                <input type="radio" name="server_type" id="st_cloud" value="cloud" {{ old('server_type', $server->server_type) === 'cloud' ? 'checked' : '' }} class="status-pill-input">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="status-pills">
                                <input type="radio" name="physical_status" id="ps_active" value="active" {{ old('physical_status', $server->physical_status) === 'active' ? 'checked' : '' }} class="status-pill-input">
                                <label for="ps_active" class="status-pill status-pill-success">Active</label>
                                <input type="radio" name="physical_status" id="ps_maintenance" value="maintenance" {{ old('physical_status', $server->physical_status) === 'maintenance' ? 'checked' : '' }} class="status-pill-input">
                                <label for="ps_maintenance" class="status-pill status-pill-warning">Maintenance</label>
                                <input type="radio" name="physical_status" id="ps_inactive" value="inactive" {{ old('physical_status', $server->physical_status) === 'inactive' ? 'checked' : '' }} class="status-pill-input">
                                <label for="ps_inactive" class="status-pill status-pill-secondary">Inactive</label>
                                <input type="radio" name="physical_status" id="ps_decommissioned" value="decommissioned" {{ old('physical_status', $server->physical_status) === 'decommissioned' ? 'checked' : '' }} class="status-pill-input">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" class="form-control @error('sort_order') is-invalid @enderror" id="sort_order" name="sort_order" value="{{ old('sort_order', $server->sort_order ?? 0) }}" min="0">
                            @error('sort_order')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="d-flex gap-2 flex-wrap">
                            <button type="submit" class="btn btn-primary">Simpan</button>
                            <a href="{{ route('server.index') }}" class="btn btn-outline-secondary">Batal</a>
                            <a href="{{ route('server.show', $server) }}" class="btn btn-outline-primary ms-auto">Lihat Detail</a>
                        </div>
                    </form>
                </div>
            </div>
